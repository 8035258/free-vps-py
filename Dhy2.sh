#!/bin/bash
# Hysteria2 一键安装脚本 - Debian 11 + systemd
# 保持原功能：随机端口/密码，自动清理旧进程
...
wget -q "https://github.com/apernet/hysteria/releases/latest/download/$HY2_FILE" -O /usr/local/bin/hysteria || { echo "下载 hysteria 二进制失败"; exit 1; }
chmod +x /usr/local/bin/hysteria

# ===== 配置目录 & 证书 =====
mkdir -p /etc/hysteria
# 生成 EC 私钥和自签证书（兼容性更好，不用进程替换）
