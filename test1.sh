...
uninstall_script() {
    ...
    rm -rf "$HOME/python-xray-argo" "$NODE_INFO_FILE"
...
install_dependencies() {
    if [ "$OS_ID" = "alpine" ]; then
        ...
        apk update
        apk add --no-cache bash python3 py3-pip git unzip curl procps util-linux
    elif [ "$OS_ID" = "debian" ] || [ "$OS_ID" = "ubuntu" ]; then
        echo -e "${YELLOW}检测到 Debian/Ubuntu 系统，使用 apt-get 安装依赖...${NC}"
        export DEBIAN_FRONTEND=noninteractive
        apt-get update
        apt-get install -y python3 python3-pip git unzip curl procps uuid-runtime
    else
        echo -e "${RED}不支持的操作系统: $OS_ID${NC}"
        echo -e "${YELLOW}脚本将继续运行，但请确保已手动安装以下依赖:${NC}"
        echo -e "${YELLOW}bash, python3, pip, git, unzip, curl, pkill, ps, uuidgen${NC}"
        return
    fi
...
if [ ! -d "$PROJECT_DIR" ]; then
    ...
    if command -v git &> /dev/null; then
        ...
        wget -q https://github.com/eooce/python-xray-argo/archive/refs/heads/main.zip -O "$HOME/python-xray-argo.zip"
        unzip -q "$HOME/python-xray-argo.zip" -d "$HOME"
        mv "$HOME/python-xray-argo-main" "$PROJECT_DIR"
        rm "$HOME/python-xray-argo.zip"
    fi

    if [ $? -ne 0 ] || [ ! -d "$PROJECT_DIR" ]; then
        echo -e "${RED}下载失败，请检查网络连接或依赖项是否已安装。${NC}"
        exit 1
    fi
fi
...
if ! ps -p $APP_PID > /dev/null 2>&1; then
    # Try to find PID again if the first method failed
    APP_PID=$(pgrep -f "python3 app.py" | head -1)
fi

if [ -z "$APP_PID" ]; then
    echo -e "${RED}服务启动失败，请检查Python环境和依赖项${NC}"
...
echo -e "${YELLOW}使用脚本选择选项3或执行 bash $0 -v 可随时查看节点信息${NC}"
