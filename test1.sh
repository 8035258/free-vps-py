...
if [ "$PM" = "apt" ]; then
apt-get update
apt-get install -y python3 python3-pip git unzip curl sed
elif [ "$PM" = "apk" ]; then
apk update
apk add --no-cache python3 py3-pip git unzip curl sed
else
echo -e "${RED}不支持的系统，请手动安装依赖: python3, pip3, git, unzip, curl, sed${NC}"
exit 1
fi
}
