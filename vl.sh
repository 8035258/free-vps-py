...
# - 修复 Alpine Linux 兼容性问题 (手动安装 Xray 并配置 OpenRC)
...
echo -e "\n${GREEN}===========================================${NC}"
echo -e "${GREEN}1. 系统检测与依赖安装 (curl, wget, socat, openssl)${NC}"
echo -e "${GREEN}===========================================${NC}"
...
if grep -qs "Debian GNU/Linux 11" /etc/os-release; then
    ...
    echo -e "${GREEN}安装依赖...${NC}"
    # Alpine 需要额外安装 unzip 来解压 Xray 文件
    apk update
...
echo -e "\n${GREEN}===========================================${NC}"
echo -e "${GREEN}2. Xray Core 安装${NC}"
echo -e "${GREEN}===========================================${NC}"

if [ "$OS" == "Debian_11" ]; then
    # Debian (Systemd) - 使用官方脚本
    echo -e "${YELLOW}Debian 使用官方一键安装脚本...${NC}"
    ...
    wget -q --show-progress -O "${TEMP_DIR}/xray.zip" "$DOWNLOAD_URL" || { echo -e "${RED}Xray 下载失败。请检查架构是否为 x86_64。${NC}"; rm -rf "${TEMP_DIR}"; exit 1; }
...
# SNI 设置为 IP 地址 (匹配自签名证书 CN)，同时添加 allowInsecure=true 适配客户端
