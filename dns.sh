...
# 添加公共DNS配置（单行JSON）
# ========================
echo -e "${BLUE}配置公共DNS...${NC}"

DNS_CONFIG='{"dns":{"servers":["1.1.1.1","8.8.8.8","8.8.4.4"]},"routing":{"rules":[{"type":"field","outboundTag":"dns","domain":["geosite:geolocation-!cn"]}]}}'

if ! grep -q '"dns"' app.py; then
    # 在 outbounds 前插入
    sed -i "/outbounds/ a $DNS_CONFIG" app.py
    echo -e "${GREEN}公共DNS已配置: 1.1.1.1, 8.8.8.8, 8.8.4.4${NC}"
else
    echo -e "${YELLOW}DNS配置已存在，跳过${NC}"
fi
