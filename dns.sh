...
# 修改或插入 DNS 配置
if grep -q "dns\s*=" app.py; then
    sed -i "s|dns\s*=.*|dns = {\"servers\": [\"1.1.1.1\", \"1.0.0.1\"]}|g" app.py
else
    sed -i "/config\s*=\s*{/a \    \"dns\": {\"servers\": [\"1.1.1.1\", \"1.0.0.1\"]}," app.py
fi
echo -e "${GREEN}服务端已固定使用公共 DNS (1.1.1.1 / 1.0.0.1)${NC}"
